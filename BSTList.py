# the same code as for AVLTreeList, only the rotation methods are empty,
# fixTree() only updates height, size
# and there's a pointer to the first node.


"""A class represnting a node in an BST tree"""
...
"""
    ...
    """
       ...
       @param child: bst node. to be the right child of self.
        """
    ...
    """
       ...
       @param child: bst node. to be the left child of self.
        """
...
"""
A class implementing the ADT list, using an BST tree.
"""


class BSTList(object):
    virtualNode = BSTNode(None, -1, 0)  # class attribute for the sentinel object
...
"""
    ...
    def treeSelect(self, i):
        ...
        return treeSelectRec(self.root, i) if i > 1 else self._first

    """performed right rotation for AVL balancing. now does nothing.
    ...
    """performed left rotation for AVL balancing. now does nothing.
    ...
    """ballanced the path between y to root by AVL rules and updated size & height.
       now only updates size & height.
    ...
    def fixTree(self, y):
        ...
        while y is not None:
            ...
            if height_changed: changes += 1  # if height changed , add 1 change to count
            y = y.getParent()
        return changes
	...
	@rtype: list
	@returns: the number of rebalancing operations (height changes)
	"""
...
"""
    ...
    @type other: BSTList
    @param other: an BST tree to be joined with self
    """
	...
	@rtype: list
	@returns: a list [left, val, right], where left is an BSTList representing the list until index i-1,
	right is an BSTList representing the list from index i+1, and val is the value at the i'th index.
	"""
        ...
        if nodeX != self.root:
            ...
            while parent is not None:
                if parent.getLeft() == curNode:  # turned right
                    ...
                    leftRightTree = BSTList()  # to be joined to a left sub-tree after turning left
                    if not left.empty():
	...
	@rtype: int
	@returns: the absolute value of the difference between the height of the BST trees joined
	"""
    ...
    """update root to be new_root
       ...
       @param new_root: BST node. to be the new root of self.
    """
